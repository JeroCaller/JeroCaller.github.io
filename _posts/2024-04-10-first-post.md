...
안녕하세요. 첫 블로그 포스팅[^1]입니다. 테스트용으로 이 글을 적어봅니다[^2]. 
...
---
[^1]: 로컬에서 블로그를 실행해보려면 명령어 창에서 `bundle exec jekyll serve` 입력 후 메시지에 뜨는 localhost 주소를 웹 브라우저에 띄우면 됩니다. 
[^2]: 각주 설명 테스트입니다. littlefoot.js 라이브러리를 이용하였습니다. 마크다운 문법인 `[^1]` , `[^1] : 각주 내용` 과 같은 형태로 각주를 작성하기만 하면 됩니다. 참고로 littlefoot.js 사용 시 문서 맨 밑에 작성하는 각주 내용은 별도로 출력되지 않으니 참고바랍니다.