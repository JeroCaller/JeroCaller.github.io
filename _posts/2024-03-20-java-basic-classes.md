...
Number 클래스는 래퍼 클래스들 중, 숫자형 래퍼 클래스들이 상속하는 추상 클래스이다. 해당 클래스에는 어떤 추상 메서드가 있고, 이 클래스를 상속받는 클래스들에는 각각 shortValue(), intValue(), doubleValue() 등의 메서드가 존재한다.  해당 메서드에 다른 자료형의 값을 인자로 대입하면, 그 값을 해당 객체로 변환해주는 기능을 한다. 예를 들어 기본 int형 값인 10을 `Integer.intValue(10)` 메서드에 대입하면 10을 가지는 Integer 객체형으로 변환해준다. 이러한 방식을 이용하면 숫자형 래퍼 클래스형 변수 선언과 동시에 할당도 가능하다. 
