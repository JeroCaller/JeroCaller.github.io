...
변수 자료형 이름 뒤에 화살괄호 <>를 적은 후, 그 안에 원하는 자료형을 대입한다. 이 때, 자료형의 이름과 화살괄호 부분을 묶어 “매개변수화 타입(Parameterized type)”이라 부른다. 그 후, 해당 변수에 객체를 생성해 대입하는 부분에서는 객체명과 괄호 () 사이에 화살괄호 <>를 적는다. 여기에 자료형을 적어넣으면 된다. 그런데 어차피 매개변수화 타입에서 그 자료형을 언급했기에 자바 7 버전 이후부터는 생략이 가능하다. 이렇게 객체를 생성하는 부분에서의 화살괄호 부분을 타입 인수(Type argument)라고 한다. 다시 정리하면 다음과 같다. 
